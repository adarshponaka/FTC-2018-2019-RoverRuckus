...
public abstract class adafruitbno055 implements HardwareDevice, I2cController.I2cPortReadyCallback{
    ...
    public adafruitbno055 (HardwareMap currentHWmap, String configuredIMUname,
    ...
    private boolean i2cWriteImmediately(byte[]outboundBytes, int byteCount, int registerAddress){
        ...
        i2cIMU.enableI2cWriteMode(j, registerAddress,byteCount);
    ...
    private boolean autoCalibrationOK(int timeOutSeconds){
        ...
        while ((System.nanoTime() - calibrationStart) <= 60000000000L) {//Set a 1-minute overall timeout
            ...
            if (!readingEnabled) {//Start a stream of reads of the calibration status byte
                i2cIMU.enableI2cReadMode(j, BNO055_CALIB_STAT_ADDR, 2);
    ...
    public void startIMU(){
        ...
        i2cIMU.enableI2cReadMode(j, registerStartAddress, numberOfRegisters);
        maxReadInterval = 0.0;
        avgReadInterval = 0.0;
        totalI2Creads = 0L;
        i2cIMU.setI2cPortActionFlag();//Set this flag to do the next read
